...
function readOnlyWallet(publicKey: PublicKey): AnchorWalletLike {
  return { publicKey, signTransaction: async <T>(tx: T) => tx, signAllTransactions: async <T>(txs: T[]) => txs };
}
...
export async function POST(req: Request) {
  ...
  let pactId: bigint;
  try { pactId = BigInt(pact_id); } catch {
    return NextResponse.json({ ok: false, error: "INVALID_PACT_ID" }, { status: 400 });
  }
  if (!fixture_id || !statement || !terms || !stake_usdc || pactId < 0n || typeof creator_wallet !== "string") {
    return NextResponse.json(
      { ok: false, error: "Missing required fields: pact_id, fixture_id, statement, terms, stake_usdc, creator_wallet" },
      { status: 400 },
    );
  }

  // The DB indexes a confirmed devnet escrow; it does not create a second, off-chain pact.
  try {
    ...
    const chainPact = await (program.account as any).pact.fetch(pactPda(pactId)) as {
      creator: PublicKey; fixtureId: { toString: () => string }; stakeAmount: { toString: () => string };
    };
    const expectedStake = BigInt(Math.round(Number(stake_usdc) * 1_000_000));
    if (!chainPact.creator.equals(creator) || chainPact.fixtureId.toString() !== String(fixture_id) || chainPact.stakeAmount.toString() !== expectedStake.toString()) {
